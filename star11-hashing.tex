...
\begin{enumerate}
    ...
    \item Sei $X\subset\mathbb N$ eine endliche Menge der Größe $|X|=k$. Sei $p_k$ die Wahrscheinlichkeit, dass \emph{keine} Kollision auftritt, das heißt:
    \[p_k=\pr_{a,b}(\forall x,y\in X\colon x=y \vee h_{a,b}(x)\ne h_{a,b}(y))\,.\]
    Beweise, dass folgende Ungleichungen gelten:
    \[
        \left(1-\frac{k}{p}\right)^k\le p_k \leq e^{-(k-1)k/(2p)}\,.
    \]
