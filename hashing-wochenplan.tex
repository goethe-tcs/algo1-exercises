...
\begin{aufgabe}[Von Hand laufen lassen und Eigenschaften]\label{tue-first}\mbox{}
	\begin{enumerate}
		...
		\item\label{hand-sondieren} (\warmup) Füge die Schlüsselsequenz $2, 32, 43, 16, 77, 51, 1, 17, 42, 111$ in eine Hashtabelle der Länge 17 ein, die lineares Sondieren und die Hashfunktion $f(k) = k \mod 17$ benutzt.
		\item Lösche 111 und 51 aus der in~\ref{hand-sondieren} erzeugten Hashtabelle.
		\item\label{wrong-delete} Angenommen, wir löschen ein Element~$x$ bei linearem Sondieren, ohne die Elemente im Cluster rechts von~$x$ wieder neu einzufügen.
...
\begin{aufgabe}[Teiler in der Divisionsmethode]
	Betrachte die Hashfunktion $h(k) = k \bmod 10$ und die Schlüsselsequenz $K = 0, 5, 20, 40, 65, 15, 90, 95, 80, 55$.
	\begin{enumerate}
		\item Warum ist diese Wahl der Hashfunktion problematisch für $K$?
		\item Bei der \emph{Divisionsmethode} wählen wir die Hashfunktion als $h(k)=k\bmod m$. Erkläre, warum wir für~$m$ eine Primzahl bevorzugen.
	\end{enumerate}
\end{aufgabe}

\begin{aufgabe}[Faules Löschen bei linearem Sondieren]
	Die Methode aus~\ref{tue-first}\ref{wrong-delete} hat nicht funktioniert, wir versuchen es also nochmal anders.
