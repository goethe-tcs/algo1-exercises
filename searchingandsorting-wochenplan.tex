...
Der \emph{Median} von $A$ ist die Zahl von Rank $\lfloor(n-1)/2\rfloor$.
\begin{enumerate}
...
\begin{enumerate}[resume]
	\item Gib einen Algorithmus an, der eine Partition von $A$ in Zeit $O(n)$ ausrechnen.
	\item (schwer) Gib einen Algorithmus an, der $A$ in Zeit $O(n\log n)$ sortiert, indem er rekursiv partitioniert.
	\item (sehr schwer) Gib einen Algorithmus mit Laufzeit $O(n)$ an, der $A$ und $k$ als Eingabe nimmt und die Zahl in $A$ ausgibt, die Rank $k$ hat.
\end{enumerate}
\end{aufgabe}
