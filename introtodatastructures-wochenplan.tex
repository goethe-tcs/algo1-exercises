...
\begin{aufgabe}[Eine Liste umkehren]
	Entwirf einen Algorithmus, der eine verkettete Liste $L_{\text{in}}$ mit $n$ Elementen als Eingabe erhält und eine verkette Liste $L_{\text{out}}$ ausgibt, die die Elemente $L_{\text{in}}$ in umgekehrter Reihenfolge enthält.
	Der Algorithmus soll $O(n)$ Zeit benötigen und dabei, abgesehen von den beiden Listen, nicht mehr als konstant viel zusätzlichen Speicherplatz benötigen\footnote{Das heißt, der zusätzliche Speicherbedarf muss durch eine Konstante (unabhängig von $n$) beschränkt bleiben, selbst dann, wenn $n$ beliebig wächst.}.
\end{aufgabe}
